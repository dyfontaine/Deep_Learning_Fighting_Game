...
#%%
class ProjectConfig:
    basepath      = "film_vgg_input-copy2"
    imsize        = (112,) * 2 # n x n square images, VGG default is 224x224
    tsize         = imsize + (3,)
...
cfg.fclayers      = 3       # Number of FC layers
cfg.fcdropout     = 0.2      # Dropout regularization factor for FC layers
...
# Optimizer settings
optimizer = Adam()   
cfg.batch_size, cfg.nb_epoch = 32, 500 # Change for early stopping regularization
# original 2000
...
datagen = ImageDataGenerator(
    ...
    height_shift_range=0.0,                 # randomly shift icvmages vertically (fraction of total height)
...
def makedata(basepath):
    ...
    for (X_data, y_data, bp) in [(X_train, y_train, cfg.trainfolder), (X_test, y_test, cfg.testfolder)]:
        ...
        for i, (im, lbl) in tqdm.tqdm(enumerate(zip(X_data, y_data)), desc='Making data folder', total=len(y_data)):
            ...
            if not os.path.exists(pn): scipy.misc.imsave(pn, scipy.misc.imresize(im, cfg.imsize, interp='nearest'))
    ...
    for root, dirs, files in tqdm.tqdm(os.walk(basepath), mininterval=3, desc='Loading batch data', total=len(obj_classes)):
        ...
        for i, f in enumerate(random.sample(files, min(len(files), samples_per_class))):
            ...
            try: 
                ...
                im = scipy.misc.imresize(im, cfg.imsize, interp='nearest')
...
if cfg.vgglayers >= 1: # Block 1
    x = Convolution2D(64, (2, 2), activation='relu', padding='same', name='block1_conv1')(img_input)
    x = Convolution2D(64, (2, 2), activation='relu', padding='same', name='block1_conv2')(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='block1_pool')(x)
    if cfg.batchnorm: x = BatchNormalization()(x)
if cfg.vgglayers >= 2: # Block 2
    x = Convolution2D(128, (2, 2), activation='relu', padding='same', name='block2_conv1')(x)
    x = Convolution2D(128, (2, 2), activation='relu', padding='same', name='block2_conv2')(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='block2_pool')(x)
    if cfg.batchnorm: x = BatchNormalization()(x)
if cfg.vgglayers >= 3: # Block 3
    x = Convolution2D(256, (2, 2), activation='relu', padding='same', name='block3_conv1')(x)
    x = Convolution2D(256, (2, 2), activation='relu', padding='same', name='block3_conv2')(x)
    x = Convolution2D(256, (2, 2), activation='relu', padding='same', name='block3_conv3')(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='block3_pool')(x)
    if cfg.batchnorm: x = BatchNormalization()(x)
if cfg.vgglayers >= 4: # Block 4
    x = Convolution2D(512, (2, 2), activation='relu', padding='same', name='block4_conv1')(x)
    x = Convolution2D(512, (2, 2), activation='relu', padding='same', name='block4_conv2')(x)
    x = Convolution2D(512, (2, 2), activation='relu', padding='same', name='block4_conv3')(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='block4_pool')(x)
    if cfg.batchnorm: x = BatchNormalization()(x)
if cfg.vgglayers >= 5: # Block 5
    x = Convolution2D(512, (2, 2), activation='relu', padding='same', name='block5_conv1')(x)
    x = Convolution2D(512,(2, 2), activation='relu', padding='same', name='block5_conv2')(x)
    x = Convolution2D(512, (2, 2), activation='relu', padding='same', name='block5_conv3')(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='block5_pool')(x)
    if cfg.batchnorm: x = BatchNormalization()(x)
    
x = Flatten(name='flatten')(x)
for i in range(cfg.fclayers): x = Dense(cfg.fclayersize, kernel_regularizer=l1_l2(cfg.l1, cfg.l2))(x)
x = Dense(len(obj_classes), activation='softmax', name='predictions')(x)

inputs = img_input
model = Model(inputs, x, name='vgg16')
top2_acc = lambda x, y: top_k_categorical_accuracy(x, y, k=2)
model.compile(loss='categorical_crossentropy', optimizer=optimizer, metrics=[top2_acc])
model.summary()
...
def viz_losses(stats): 
    ...
    ax1.set_yscale('log')
    ax2.plot(stats['Top 2 Accuracy'].values, label='Test accuracies', color='red')
    ax2.axhline(1.0/len(obj_classes), linestyle='dashed', color='gray')
    ax2.text(0, 1.0/len(obj_classes), 'Chance')
    ax2.axhline(np.max(stats['Top 2 Accuracy']), linestyle='dashed', color='red')
    ax2.text(0, np.max(stats['Top 2 Accuracy']), 'Best')    
    ax2.set_ylim([0, 1])
    ax2.set_title('Top 2 Accuracy: %0.2f%%' % (100.0*stats['Top 2 Accuracy'].values[-1]))     
...
class Heatmap:
    ...
    def explain_prediction_heatmap(self, im, actual):
        ...
        plt.imshow(hsv(im)), plt.xticks([]), plt.yticks([]), plt.title('Full image'), plt.show()
...
#%% Training code
datagen.fit(X_test)
cfg.modelid = os.path.join(cfg.basepath, 'model-top2.h5')
...
trainstats = pd.DataFrame(columns=('Train loss', 'Test loss', 'Top 2 Accuracy'))
...
for e in range(cfg.nb_epoch):
    ...
    trainstats.loc[len(trainstats)] = (loss.history['loss'][0], loss.history['val_loss'][0], loss.history['val_<lambda>'][0])
